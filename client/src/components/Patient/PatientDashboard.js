...
function PatientDashboard({ account, web3 }) {
  ...
  if (loading) {
    return (
      <div style={S.center}>
        ...
        <h2 style={{ color: "#2c3e50" }}>Loading dashboard...</h2>
  ...
  if (notRegistered) {
    return (
      <div style={S.page}>
        <div style={S.header}>
          <h1 style={S.title}>🧑‍⚕️ Patient Dashboard</h1>
          <p style={S.badge}>{account}</p>
        </div>
        <div style={S.emptyCard}>
          <div style={{ fontSize: "60px" }}>📋</div>
          <h2 style={{ color: "#2c3e50", marginBottom: "10px" }}>Not Registered Yet!</h2>
    ...
    <div style={S.page}>
      ...
      <div style={S.row}>
        ...
        <div style={S.card}>
          ...
          <div style={{
            ...S.statusBanner,
            backgroundColor:
              patient.status === "Approved" ? "#eafaf1"
              : patient.status === "Rejected" ? "#fdf2f2"
              : "#fff8e1",
            borderColor:
              patient.status === "Approved" ? "#2ecc71"
              : patient.status === "Rejected" ? "#e74c3c"
              : "#f39c12",
          }}>
            ...
            <div>
              ...
              <div style={{
                fontSize: "13px",
                color:
                  patient.status === "Approved" ? "#27ae60"
                  : patient.status === "Rejected" ? "#e74c3c"
                  : "#f39c12",
              }}>
                {patient.status}
          ...
          {patient.status === "Approved" && subscription && (
            <div style={S.policyMiniBox}>
              ...
              <span style={{
                ...S.pill,
                backgroundColor:
                  subscription.subscriptionStatus === "Active" ? "#d4edda"
                  : subscription.subscriptionStatus === "Suspended" ? "#fff3cd"
                  : "#f8d7da",
                color:
                  subscription.subscriptionStatus === "Active" ? "#155724"
                  : subscription.subscriptionStatus === "Suspended" ? "#856404"
                  : "#721c24",
              }}>
          ...
          {patient.status === "Rejected" && (
            <div style={S.rejectedBox}>
              <p style={{ color: "#721c24", fontSize: "14px", lineHeight: "1.6" }}>
      ...
      {subscription && (
        <div style={S.row}>
          ...
          {/* Policy Details */}
          <div style={S.card}>
            <h2 style={S.cardTitle}>🛡️ My Policy</h2>

            <div style={{
              ...S.statusBanner,
              backgroundColor:
                subscription.subscriptionStatus === "Active"    ? "#eafaf1"
                : subscription.subscriptionStatus === "Suspended" ? "#fff3cd"
                : "#fdf2f2",
              borderColor:
                subscription.subscriptionStatus === "Active"    ? "#2ecc71"
                : subscription.subscriptionStatus === "Suspended" ? "#f39c12"
                : "#e74c3c",
              marginBottom: "15px",
            }}>
              <span style={{ fontSize: "22px" }}>
                {subscription.subscriptionStatus === "Active"    ? "✅"
                 : subscription.subscriptionStatus === "Suspended" ? "⚠️" : "❌"}
              </span>
              <div>
                <div style={{ fontWeight: "bold", fontSize: "15px", color: "#2c3e50" }}>
                  {subscription.policyName}
          ...
          <div style={S.card}>
            ...
            <div style={{
              ...S.dueBanner,
              backgroundColor:
                subscription.paymentStatus === "Paid"    ? "#eafaf1"
                : subscription.paymentStatus === "Due"   ? "#fff3cd"
                : "#fdf2f2",
            }}>
              <div style={{ fontSize: "32px", fontWeight: "800", color: "#2c3e50" }}>
              ...
              <div style={{
                ...
                color:
                  subscription.paymentStatus === "Paid"    ? "#27ae60"
                  : subscription.paymentStatus === "Due"   ? "#856404"
                  : "#721c24",
              }}>
            ...
            {subscription.subscriptionStatus !== "Expired" && (
              <button
                style={{
                  ...S.payBtn,
                  backgroundColor:
                    subscription.paymentStatus === "Paid" ? "#95a5a6" : "#e74c3c",
                }}
              ...
              >
                {paying
                  ...
                  : `⚠️ Pay Now — ${formatETH(subscription.premiumAmount)} ETH`}
              </button>
            )}
                ...
                <tbody>
                  {paymentHistory.map((p, i) => (
                    <tr key={i} style={{ backgroundColor: i % 2 === 0 ? "#fff" : "#f9fafb" }}>
                      ...
                      <td style={{ ...S.td, fontWeight: "600", color: "#27ae60" }}>
                        {formatETH(p.amount)} ETH
                      </td>
                      <td style={S.td}>{formatDate(p.paidOn)}</td>
                      <td style={S.td}>
                        <span style={{ ...S.pill, backgroundColor: "#d4edda", color: "#155724" }}>
                          ✅ {p.status}
                        </span>
                      </td>
                    </tr>
...
function InfoRow({ label, value }) {
  return (
    <div style={{
      ...
      <span style={{ color: "#7f8c8d", fontSize: "13px", fontWeight: "bold" }}>{label}</span>
      <span style={{ color: "#2c3e50", fontSize: "13px", textAlign: "right", maxWidth: "60%" }}>{value}</span>
    </div>
  );
}
...
const S = {
  ...
  refreshBtn:   { backgroundColor: "#95a5a6", color: "white", padding: "7px 16px", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "13px", display: "block", margin: "8px auto 0" },
  successMsg:   { color: "#27ae60", backgroundColor: "#eafaf1", padding: "12px", borderRadius: "8px", textAlign: "center", maxWidth: "900px", margin: "0 auto 15px auto" },
  errorMsg:     { color: "#e74c3c", backgroundColor: "#fdf2f2", padding: "12px", borderRadius: "8px", textAlign: "center", maxWidth: "900px", margin: "0 auto 15px auto" },
  ...
  cardTitle:    { fontSize: "17px", color: "#2c3e50", marginBottom: "18px", fontWeight: "700" },
  ...
  noPolicyBox:  { backgroundColor: "#f8f9fa", padding: "20px", borderRadius: "10px", textAlign: "center", marginTop: "10px" },
  ...
  rejectedBox:  { backgroundColor: "#fdf2f2", padding: "15px", borderRadius: "10px", marginTop: "10px" },
  expiredBox:   { backgroundColor: "#fdf2f2", padding: "15px", borderRadius: "10px", textAlign: "center", marginTop: "10px" },
  ...
  statBox:      { flex: 1, backgroundColor: "#f8f9fa", padding: "12px", borderRadius: "10px", textAlign: "center" },
  statNum:      { fontSize: "20px", fontWeight: "800", color: "#2c3e50" },
  statLabel:    { fontSize: "11px", color: "#7f8c8d", marginTop: "3px" },
  progressTrack:{ height: "8px", backgroundColor: "#f0f4f8", borderRadius: "10px", overflow: "hidden" },
  progressBar:  { height: "100%", backgroundColor: "#2ecc71", borderRadius: "10px" },
  btn:          { backgroundColor: "#3498db", color: "white", padding: "11px 20px", border: "none", borderRadius: "8px", cursor: "pointer", fontSize: "14px", fontWeight: "bold", width: "100%" },
  pill:         { padding: "3px 10px", borderRadius: "20px", fontSize: "11px", fontWeight: "bold" },
  tableWrap:    { overflowX: "auto" },
  table:        { width: "100%", borderCollapse: "collapse" },
  th:           { backgroundColor: "#f8f9fa", padding: "11px 16px", textAlign: "left", fontSize: "11px", fontWeight: "700", color: "#6c757d", textTransform: "uppercase", letterSpacing: "0.05em", borderBottom: "2px solid #e9ecef" },
